...
void UGrandCityMobileAuthWidget::BuildInterface()
{
    ...
    Panel->AddChildToVerticalBox(DisplayNameInput);

    PasswordInput = WidgetTree->ConstructWidget<UEditableTextBox>(UEditableTextBox::StaticClass());
    PasswordInput->SetHintText(FText::FromString(TEXT("Password")));
    PasswordInput->SetIsPassword(true);
    Panel->AddChildToVerticalBox(PasswordInput);

    ConfirmPasswordInput = WidgetTree->ConstructWidget<UEditableTextBox>(UEditableTextBox::StaticClass());
    ConfirmPasswordInput->SetHintText(FText::FromString(TEXT("Confirm password")));
    ConfirmPasswordInput->SetIsPassword(true);
    ConfirmPasswordInput->SetVisibility(ESlateVisibility::Collapsed);
    Panel->AddChildToVerticalBox(ConfirmPasswordInput);

    PrimaryButton = WidgetTree->ConstructWidget<UButton>(UButton::StaticClass());
    UTextBlock* PrimaryLabel = WidgetTree->ConstructWidget<UTextBlock>(UTextBlock::StaticClass());
    PrimaryLabel->SetText(FText::FromString(TEXT("LOGIN")));
    PrimaryButton->AddChild(PrimaryLabel);
    Panel->AddChildToVerticalBox(PrimaryButton);
    PrimaryButton->OnClicked.AddDynamic(this, &UGrandCityMobileAuthWidget::HandleLoginClicked);

    ModeButton = WidgetTree->ConstructWidget<UButton>(UButton::StaticClass());
    ModeText = WidgetTree->ConstructWidget<UTextBlock>(UTextBlock::StaticClass());
    ModeText->SetText(FText::FromString(TEXT("Create a new account")));
    ModeButton->AddChild(ModeText);
    Panel->AddChildToVerticalBox(ModeButton);
...
void UGrandCityMobileAuthWidget::HandleLoginClicked()
{
    ...
    {
        SetStatus(TEXT("Enter your display name and password."), true);
        return;
    }
    ...
    AccountClient->LoginAccount(DisplayName, Password,
        FGrandCityAccountClientResult::CreateUObject(this, &UGrandCityMobileAuthWidget::CompleteAuthentication));
}
...
void UGrandCityMobileAuthWidget::HandleRegisterClicked()
{
    ...
    const FString DisplayName = DisplayNameInput->GetText().ToString().TrimStartAndEnd();
    const FString Password = PasswordInput->GetText().ToString();
    const FString Confirm = ConfirmPasswordInput->GetText().ToString();
    ...
    AccountClient->RegisterAccount(DisplayName, Password,
        FGrandCityAccountClientResult::CreateUObject(this, &UGrandCityMobileAuthWidget::CompleteAuthentication));
}
...
{
    bBusy = bInBusy;
    if (PrimaryButton) PrimaryButton->SetIsEnabled(!bBusy);
    if (ModeButton) ModeButton->SetIsEnabled(!bBusy);
}
...
void UGrandCityMobileAuthWidget::CompleteAuthentication(bool bSuccess, const FGrandCityAccountIdentity& Identity, const FString& AuthToken, const FString& ErrorCode)
{
    ...
    if (!bSuccess)
    {
        SetStatus(FString::Printf(TEXT("Authentication failed: %s"), *ErrorCode), true);
        return;
    }
